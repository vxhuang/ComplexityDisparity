...
get_plotlist <- function(pol_df, pol_type) {
  ...
  race_df$scenario <- recode(race_df$scenario,
                             'ref2050_2017' = 'Reference 2050',
                             'nz2050_2017' = 'Net-Zero 2050',
                             'ref2017' = '2017',
                             'elec_2017' = 'Net-Zero 2050 (ELE)',
                             'SSP3' = 'Net-Zero 2050 (SSP3)',
                             'nz_equity_2017' = 'Net-Zero 2050 (Equity)',
                             'ref_2016meteo' = '2017 (2016 MET)',
                             'nz_2016meteo' = 'Net-Zero 2050 (2016 MET)')
  ...
  urban_df$style <- ifelse(urban_df$scenario %in% c('2017', 'Reference 2050', 'Net-Zero 2050', '2017 (2016 MET)', 'Net-Zero 2050 (2016 MET)'), 'Main', 'Sensitivity')
  income_df$style <- ifelse(income_df$scenario %in% c('2017', 'Reference 2050', 'Net-Zero 2050', '2017 (2016 MET)', 'Net-Zero 2050 (2016 MET)'), 'Main', 'Sensitivity')
  race_df$style <- ifelse(race_df$scenario %in% c('2017', 'Reference 2050', 'Net-Zero 2050', '2017 (2016 MET)', 'Net-Zero 2050 (2016 MET)'), 'Main', 'Sensitivity')
  
  final_scenario_list <- c('2017', '2017 (2016 MET)', 'Net-Zero 2050', 'Net-Zero 2050 (2016 MET)')
  
  race_df$scenario <- fct_relevel(race_df$scenario, final_scenario_list)
  urban_df$scenario <- fct_relevel(urban_df$scenario, final_scenario_list)
  income_df$scenario <- fct_relevel(income_df$scenario, final_scenario_list)
  
  print(count(race_df, scenario))
  ...
  race_df$race <- fct_relevel(race_df$race, 'Black', 'White', 'Other')
  ...
  if (pol_type == 'pm') {
    gg_race <- gg_race +
      ...
      annotate("text", x = 1, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == '2017')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 2, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == '2017 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 3, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 4, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
      ggtitle('a) Distribution of PM2.5 Across Racial Groups')
    gg_urban <- gg_urban +
      annotate("text", x = 1, y = 1, label = as.character(sprintf('%.1f', round(filter(urban_disp, scenario == '2017')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 2, y = 1, label = as.character(sprintf('%.1f', round(filter(urban_disp, scenario == '2017 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 3, y = 1, label = as.character(sprintf('%.1f', round(filter(urban_disp, scenario == 'Net-Zero 2050')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 4, y = 1, label = as.character(sprintf('%.1f', round(filter(urban_disp, scenario == 'Net-Zero 2050 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
      ggtitle('c) Distribution of PM2.5 Across Geography')
    gg_income <- gg_income +
      annotate("text", x = 1, y = 1, label = as.character(sprintf('%.1f', round(filter(income_disp, scenario == '2017')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 2, y = 1, label = as.character(sprintf('%.1f', round(filter(income_disp, scenario == '2017 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 3, y = 1, label = as.character(sprintf('%.1f', round(filter(income_disp, scenario == 'Net-Zero 2050')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 4, y = 1, label = as.character(sprintf('%.1f', round(filter(income_disp, scenario == 'Net-Zero 2050 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
      ggtitle('b) Distribution of PM2.5 Across Income Groups')
  ...
  if (pol_type == 'ozone') {
    gg_race <- gg_race + 
      ...
      annotate("text", x = 4, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
      ggtitle('d) Distribution of Ozone Across Racial Groups')
    gg_urban <- gg_urban +
      annotate("text", x = 1, y = 1, label = as.character(sprintf('%.1f', round(filter(urban_disp, scenario == '2017')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 2, y = 1, label = as.character(sprintf('%.1f', round(filter(urban_disp, scenario == '2017 (2016 MET)')$med_diff, 1))),
      ...
      annotate("text", x = 4, y = 1, label = as.character(sprintf('%.1f', round(filter(urban_disp, scenario == 'Net-Zero 2050 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
      ggtitle('f) Distribution of Ozone Across Geography')
    gg_income <- gg_income +
      annotate("text", x = 1, y = 1, label = as.character(sprintf('%.1f', round(filter(income_disp, scenario == '2017')$med_diff, 1))),
               col = 'Black', size = 3) +
      annotate("text", x = 2, y = 1, label = as.character(sprintf('%.1f', round(filter(income_disp, scenario == '2017 (2016 MET)')$med_diff, 1))),
      ...
      annotate("text", x = 4, y = 1, label = as.character(sprintf('%.1f', round(filter(income_disp, scenario == 'Net-Zero 2050 (2016 MET)')$med_diff, 1))),
               col = 'Black', size = 3) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
      ggtitle('e) Distribution of Ozone Across Income Groups')
  }
...
jpeg(paste0(plot_dir, '/fig4_met_sensitivity_', Sys.Date(), '.jpg'), 
     width = 20, height = 9, quality = 1, units = 'in', res = 300)
print(ggarrange(plotlist = all_plots))
dev.off()

pdf(paste0(plot_dir, '/fig4_met_sensitivity_', Sys.Date(), '.pdf'), width = 18, height = 8.5)
print(ggarrange(plotlist = all_plots))
dev.off()
