...
get_wei_plot_race <- function(demo, poll) {
	...
	wei_plot_df_plot_v2 <- wei_plot_df_plot %>%
		...
		filter(scenario %in% c('2017', 'equityNZ2050scaled')) %>%
		pivot_wider(names_from = scenario, values_from = disparity) %>%
		filter(!is.na(STNAME))
	
	wei_plot_df_plot_v2$direction <- ifelse(wei_plot_df_plot_v2$`2017` > wei_plot_df_plot_v2$equityNZ2050scaled,
		'decreasing', 'increasing')
	
	gg_wei2 <- ggplot(wei_plot_df_plot_v2) +
					geom_vline(xintercept = 0) +
		geom_segment(aes(x = `2017`, xend = equityNZ2050scaled, y = fct_reorder(STNAME, `2017`)), 
		...
		gg_wei2 <- gg_wei2 +
			...
			theme(legend.position = 'none') +
			  ...
			  annotate("segment", x = 1.1, y = 2, xend = 1.2, yend = 2,
			           arrow = arrow(type = 'closed', length = unit(0.015, 'npc'))) +
			  annotate("text", x = 1.3, y = 2, hjust = 0, label = "Changes in Net-Zero 2050 (Equity)", size = 4)  
...
get_wei_plot_income <- function(demo, poll) {
	...
	wei_plot_df_plot_v2$direction <- ifelse(wei_plot_df_plot_v2$equityNZ2050scaled > wei_plot_df_plot_v2$`2017`,
...
jpeg(paste0('../air_health/plots/rtr_figs', '/rtr_fig5_si2_', Sys.Date(), '.jpg'), width = 16, height = 16, quality = 1, units = 'in', res = 300)
