libs <- c("dplyr", "tidyr", "ggplot2", "grid", "ggtext", "scales", "ggpubr", "ggnewscale", "geojsonio")
...
spdf <- geojson_read("./input_data/air_health/data/us_states_hexgrid.geojson", what = "sp")
...
pm_dp_all <- read.csv("./input_data/air_health/data/wrf_data/pm_disparity_race.csv")
...
pm_dp_ssp3 <- read.csv("./input_data/air_health/data/wrf_data/pm_disparity_race_ssp3.csv")
...
# use 2% max state-level disparity as the threshold
pm_dp_all <- pm_dp_all %>% select(-pengfei_2017)
pm_dp_all <- pm_dp_all %>%
  rowwise() %>%
  mutate(all_equity = case_when(
    all(c_across(2:5) >= 0.05) ~ 1,
    all(c_across(2:5) <= -0.05) ~ -1,
    all(abs(c_across(2:5) < 0.05)) ~ 0,
    TRUE ~ 0.5
  )) %>%
  ungroup()
...
oz_dp_all <- read.csv("./input_data/air_health/data/wrf_data/oz_disparity_race.csv")

oz_dp_all <- oz_dp_all %>% select(STNAME, scenario, black, total, white) %>% 
  filter(scenario %in% c("pengfei_2017", "pengfei_NZ2050", "jie_equityNZ2050scaled", "pengfei_NZ2050RCP"), 
         !STNAME %in% c("Alaska", "Hawaii", "Puerto Rico", "NA")) %>% drop_na()

oz_dp_ssp3 <- read.csv("./input_data/air_health/data/wrf_data/oz_disparity_race_ssp3.csv")
...
# use 2% max state-level disparity in 2017 as the threshold
oz_dp_all <- oz_dp_all %>% 
  rowwise() %>%
  mutate(all_equity = case_when(
    all(c_across(2:5) >= 0.13) ~ 1,
    all(c_across(2:5) <= -0.13) ~ -1,
    all(abs(c_across(2:5) < 0.13)) ~ 0,
    TRUE ~ 0.5
  )) %>%
  ungroup()
...
ggsave("./outputs/Figure6.pdf", plot = fig6, width = 14, height = 6)
