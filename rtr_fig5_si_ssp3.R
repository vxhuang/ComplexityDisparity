...
get_demo_df_ssp3 <- function(pollutant_df) {
	...
	demo_df <- left_join(ssp3_v2, ssp3_urban_v2) %>%
		mutate(GEOID = as.numeric(GEOID)) %>%
		left_join(dplyr::select(rename(fips_v2, GEOID = FIPS), GEOID, STNAME)) %>%
		mutate(urban = total*POPPCT_URB, rural = total - urban) %>%
		left_join(dplyr::select(pollutant_df, STNAME, GEOID = FIPS, `2017`, REF2050, NZ2050, NZ2050RCP, newNZ2050, RCP45NZ2050))
	return(demo_df)
}
...
get_wei_plot_race <- function(demo, poll) {
	...
	race_wt_df <- demo_df %>%
		dplyr::select(GEOID, STNAME, `2017`, REF2050, NZ2050, NZ2050RCP, newNZ2050, RCP45NZ2050,
			white, black, total) %>%
		pivot_longer(cols = c('white', 'black', 'total'), names_to = "race",
			values_to = 'count') %>%
		pivot_longer(cols = c('2017', 'REF2050', 'NZ2050', 'NZ2050RCP', 'newNZ2050', 'RCP45NZ2050'), names_to = "scenario",
			values_to = 'pm')
	...
	wei_plot_df <- filter(race_wt_df, scenario == '2017')
	message('beep')
	### SSP3

	if (poll == 'Ozone') {
		demo_df <- get_demo_df_ssp3(all_oz_new)

	} else {
		demo_df <- get_demo_df_ssp3(all_pm_new)
	...
	race_wt_df <- race_wt_df %>%
		...
		pivot_wider(values_from = pm, names_from = race) %>%
		mutate(disparity = (Black-White))
	wei_plot_df_ssp3 <- filter(race_wt_df, scenario == 'NZ2050') %>%
		mutate(scenario = 'NZ2050ssp3')

	wei_plot_df <- bind_rows(wei_plot_df, wei_plot_df_ssp3)
	message('beep')
	...
	wei_plot_df_plot_v2 <- wei_plot_df_plot %>%
		...
		filter(scenario %in% c('2017', 'NZ2050ssp3')) %>%
		pivot_wider(names_from = scenario, values_from = disparity) %>%
		filter(!is.na(STNAME))
	
	wei_plot_df_plot_v2$direction <- ifelse(wei_plot_df_plot_v2$`2017` > wei_plot_df_plot_v2$NZ2050ssp3,
		'decreasing', 'increasing')
	
	gg_wei2 <- ggplot(wei_plot_df_plot_v2) +
				geom_vline(xintercept = 0) +
		geom_segment(aes(x = `2017`, xend = NZ2050ssp3, y = fct_reorder(STNAME, `2017`)), 
			arrow = arrow(type = 'closed', length = unit(0.015, 'npc'))) +
		geom_point(data = filter(wei_plot_df_plot, scenario == '2017', !is.na(STNAME)),
	...
	if (poll == 'Ozone') {
		...
		gg_wei2 <- gg_wei2 +
			...
			theme(legend.position = 'none') +
			  ...
			  annotate("rect", xmin = 1, xmax = 3.25, ymin = 1, ymax = 5, fill = 'white', color = 'black') +
			  
			  annotate("point", x = 1.1, y = 4, size = 3) +
			  annotate("text", x = 1.3, y = 4, hjust = 0, label = "2017", size = 4) +
			  
			  # Custom annotation: Arrow symbol + label
			  annotate("segment", x = 1.1, y = 2, xend = 1.2, yend = 2,
			           arrow = arrow(type = 'closed', length = unit(0.015, 'npc'))) +
			  annotate("text", x = 1.3, y = 2, hjust = 0, label = "Changes in Net-Zero 2050 (SSP3)", size = 4)
	}
...
get_wei_plot_income <- function(demo, poll) {
	...
	income_wt_df <- as.data.frame(demo_df) %>%
		mutate(low_income = (i2+i3+i4+i5+i6), mid_income = (i7+i8+i9+i10+i11+i12+i13+i14+i15),
			high_income = (i16+i17)) %>%
		dplyr::select(GEOID, STNAME, `2017`, REF2050, NZ2050, NZ2050RCP, newNZ2050, RCP45NZ2050, low_income, mid_income,
			high_income) %>%
	...
	wei_plot_df <- filter(income_wt_df, Scenario == '2017')
	message('beep')
	######
	if (poll == 'Ozone') {
		demo_df <- get_demo_df_ssp3(all_oz_new)
	...
	income_wt_df <- income_wt_df %>%
		...
		pivot_wider(values_from = pm, names_from = income) %>%
		mutate(disparity = (`Low Income` - `High Income`))
	wei_plot_df_ssp3 <- filter(income_wt_df, Scenario == 'NZ2050') %>%
		mutate(Scenario = 'NZ2050ssp3')
	wei_plot_df <- bind_rows(wei_plot_df, wei_plot_df_ssp3)
	message('beep')
	...
	wei_plot_df_plot_v2$direction <- ifelse(wei_plot_df_plot_v2$NZ2050ssp3 > wei_plot_df_plot_v2$`2017`,
...
get_wei_plot_urban <- function(demo, poll) {
	...
	wei_plot_df <- filter(urban_wt_df, scenario == '2017')
...
jpeg(paste0('../air_health/plots/rtr_figs', '/rtr_fig5_si4_', Sys.Date(), '.jpg'), width = 16, height = 16, quality = 1, units = 'in', res = 300)
