...
get_race_df <- function(demo_df) {
	...
	race_wt_df <- mutate(race_wt_df, 
		scenario = fct_relevel(scenario, '2017', 'REF2050', 'NZ2050', 'NZ2050RCP', 'newNZ2050', 'RCP45NZ2050', 'equityNZ2050scaled'))
...
# 	race_disp <- bind_rows(race_disp, race_disp_ssp3, race_disp_ssp5)
	...
	race_df$scenario <- fct_relevel(race_df$scenario, '2017', 'Reference 2050', 'Net-Zero 2050', 
		'Net-Zero 2050 (RCP8.5)', 'Net-Zero 2050 (Equity)', 'Net-Zero 2050 (RCP4.5)', 'Net-Zero 2050 (Equity, old)')
	race_disp$scenario <- fct_relevel(race_disp$scenario, '2017', 'Reference 2050', 'Net-Zero 2050', 
		'Net-Zero 2050 (RCP8.5)', 'Net-Zero 2050 (Equity)', 'Net-Zero 2050 (RCP4.5)', 'Net-Zero 2050 (Equity, old)')

	race_df <- filter(race_df, scenario %in% c('2017', 'Reference 2050', 'Net-Zero 2050', 
		'Net-Zero 2050 (RCP8.5)', 'Net-Zero 2050 (Equity)'))
	race_disp <- filter(race_disp, scenario %in% c('2017', 'Reference 2050', 'Net-Zero 2050', 
		'Net-Zero 2050 (RCP8.5)', 'Net-Zero 2050 (Equity)')) %>%
		rename(race = strata)
	gg_race <- ggplot(race_df) +
		...
		facet_wrap(. ~ scenario) +
		geom_text(data = race_disp, aes(y = 1, x = race, label = round(median, 1)))
	...
	if (pol_type == 'pm') {
		gg_race <- gg_race +
			...
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 5, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (RCP8.5)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 4, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (Equity)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 6, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (RCP4.5)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 7, y = 1, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (SSP3)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			...
			# geom_text(data = race_disp, aes(x = strata, y = 1, label = round(median, 1))) +
			# labs(tag = 'a)') + 
	...
	if (pol_type == 'ozone') {
		gg_race <- gg_race + 
			...
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 5, y = 15, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (RCP8.5)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 4, y = 15, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (Equity)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 6, y = 15, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (RCP4.5)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 7, y = 15, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (SSP3)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			# annotate("text", x = 8, y = 15, label = as.character(sprintf('%.1f', round(filter(race_disp, scenario == 'Net-Zero 2050 (SSP5)')$med_diff, 1))),
			# 	col = 'Black', size = 3) +
			theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
			# geom_text(data = race_disp, aes(x = strata, y = 15, label = round(median, 1))) +
			# labs(tag = 'd)') + 
			ggtitle('b) Distribution of Ozone Exposure Across Racial Groups')
	}
...
all_plots <- list(pm_plots, oz_plots)

jpeg(paste0(plot_dir, '/fig4_si_', Sys.Date(), '.jpg'), width = 10, height = 10, quality = 1, units = 'in', res = 300)
print(ggarrange(plotlist = all_plots, ncol = 1, nrow = 2))
dev.off()
