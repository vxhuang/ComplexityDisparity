...
all_pm_new <- filter(all_data_new, TIMESCALE == 'annual', POLLUTANT == 'pm25',
	SCENARIO %in% c('2017', 'REF2050', 'NZ2050', 'NZ2050RCP', 'newNZ2050', 'RCP45NZ2050', 
		'equityNZ2050scaled')) %>%
...
get_wei_plot_race <- function(demo, poll) {
	...
	race_wt_df <- demo_df %>%
		dplyr::select(GEOID, STNAME, `2017`, REF2050, NZ2050, NZ2050RCP, 
			newNZ2050, RCP45NZ2050, equityNZ2050scaled,
			white, black, total) %>%
	...
	wei_plot_df_plot_v2 <- wei_plot_df_plot %>%
		...
		filter(scenario %in% c('2017', 'NZ2050')) %>%
		pivot_wider(names_from = scenario, values_from = disparity) %>%
		filter(!is.na(STNAME))
	
	wei_plot_df_plot_v2$direction <- ifelse(wei_plot_df_plot_v2$`2017` > wei_plot_df_plot_v2$NZ2050,
		'decreasing', 'increasing')
	
	gg_wei2 <- ggplot(wei_plot_df_plot_v2) +
		geom_vline(xintercept = 0) +
		geom_segment(aes(x = `2017`, xend = NZ2050, y = fct_reorder(STNAME, `2017`)), 
	...
	if (poll == 'Ozone') {
		...
		gg_wei2 <- gg_wei2 +
			...
			theme(legend.position = 'none')  +
			  ...
			  annotate("rect", xmin = 1, xmax = 2.75, ymin = 1, ymax = 5, fill = 'white', color = 'black') +
...
get_wei_plot_income <- function(demo, poll) {
	...
	wei_plot_df_plot_v2$direction <- ifelse(wei_plot_df_plot_v2$NZ2050 > wei_plot_df_plot_v2$`2017`,
...
get_wei_plot_urban <- function(demo, poll) {
	...
	urban_wt_df <- demo_df %>%
		dplyr::select(GEOID, STNAME, 
			`2017`, REF2050, NZ2050, NZ2050RCP, newNZ2050, RCP45NZ2050, equityNZ2050scaled,
			urban, rural, total) %>%
		pivot_longer(cols = c('urban', 'rural', 'total'), names_to = "urban",
			values_to = 'count') %>%
		pivot_longer(cols = c('2017', 'REF2050', 'NZ2050', 'NZ2050RCP', 
			'newNZ2050', 'RCP45NZ2050', 'equityNZ2050scaled'), names_to = "scenario",
			values_to = 'pm')
...
jpeg(paste0('../air_health/plots/rtr_figs', '/rtr_fig5_', Sys.Date(), '.jpg'), width = 16, height = 16, quality = 1, units = 'in', res = 300)
